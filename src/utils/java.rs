...
pub fn create_class_object(class_name: &String) -> u32 {
    ...
    let obj_id: u32 = create_object(class0.id as usize);
...
pub fn create_string_object(str_value: String) -> u32 {
    let char_array_id = {
        let chars: Vec<char> = str_value.chars().collect();
        let char_array_id: u32 = create_array(chars.len() as u32, DataType::Array { element_type: (Box::new(DataType::Char)), depth: (1) });
    ...
    let obj_id: u32 = runtime_data_area::create_object(class.id);
