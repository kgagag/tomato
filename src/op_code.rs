pub mod op_code{
    ...
    pub fn class_exists(class_name:Vec<u8>) -> bool{
        // 获取全局变量的Mutex锁
        let data: std::sync::MutexGuard<'_, UnsafeCell<HashMap<Vec<u8>, Class>>> = CLASS_DATA.lock().unwrap();
        unsafe {
    ...
    pub fn add_id_class(class_id:usize,class_name:Vec<u8>){
        // 获取全局变量的Mutex锁
        let data: std::sync::MutexGuard<'_, UnsafeCell<HashMap<usize, Vec<u8>>>> = CLASS_ID_DATA.lock().unwrap();
        // 通过UnsafeCell获取可变引用，并修改全局变量的值
        unsafe {
            // 从 UnsafeCell 中获取 HashMap 的可变引用
            let map = &mut *data.get();
    ...
    pub fn do_opcode( stack_frame: &mut StackFrame){
    ...
    pub fn istore_1(frame: &mut StackFrame) {
        let i = frame.op_stack.pop().unwrap() as u32;
        frame.local[1] = i;
        frame.pc += 1;
    }
    ...
    pub fn iload_1(frame: &mut StackFrame) {
        frame.op_stack.push(frame.local[1] as u64);
        frame.pc += 1;
    }
    
    pub fn iload_2( frame: &mut StackFrame) {
        frame.op_stack.push(frame.local[2] as u64);
        frame.pc += 1;
    }
    ...
    pub fn _new(
        ...
        let classfile_pool_index = frame.code[(frame.pc + 1) as usize];
        ...
        let len = u8s_to_u32(&class_name_utf8[1..3]);
        let class = get_or_load_class(&class_name[3..(len as usize + 3)].to_vec());
    ...
    pub fn get_class_for_invoke(frame: &StackFrame) ->usize{
        ...
        let method_ref_index = u8s_to_u16(&frame.code[1..3]) as usize;
    ...
    pub fn get_method_for_invoke( frame: &StackFrame) -> Option<&MethodInfo>{
        ...
        let name_index: usize = u8s_to_u16(&name_and_type[3..5]) as usize;
        ...
        for i in 0..class.methods_count as usize {
            ...
            //m.access_flag
            //目的是找到这个方法
            let name = &class.constant_pool[m.name_index as usize];
            let description = &class.constant_pool[m.descriptor_index as usize];
    ...
    pub fn push_stack_frame(mut stack_frame: StackFrame){
        ...
        unsafe {
            ...
            if stack_frame.vm_stack_id == 0{
                ...
                map.get_mut(&stack_frame.vm_stack_id).unwrap().push(stack_frame);
            }
            drop(data);
        }
    }

    pub fn init_stack_frame (frame:&mut StackFrame,method_info:&MethodInfo,class_id:usize) -> StackFrame{
        ...
        for param in &method_info.param {
            let v = frame.op_stack.pop().unwrap();
            match param {
                MethodParameter::Byte => {
                    new_stack_frame.local.push(v as u32);
                }
                ...
                MethodParameter::Double => {
                    ...
                    new_stack_frame.local.push(u8s_to_u32(&bytes[0..4]));
                    new_stack_frame.local.push(u8s_to_u32(&bytes[4..8]));
                }
